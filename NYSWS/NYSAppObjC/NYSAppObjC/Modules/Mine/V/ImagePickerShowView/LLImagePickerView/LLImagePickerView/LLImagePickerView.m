...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _mediaArray.count) {
        ...
        cell.LLClickDeleteButton = ^(NSIndexPath *cellIndexPath) {
            ...
            LLImagePickerModel *model = self->_mediaArray[cellIndexPath.row];
            
            if (!self->_allowMultipleSelection) {
                
                for (NSInteger idx = 0; idx < self->_selectedImageModels.count; idx++) {
                        
                    if ([((LLImagePickerModel *)self->_selectedImageModels[idx]) isEqual:model ]) {
                        [self->_selectedImageAssets removeObjectAtIndex:idx];
                        [self->_selectedImageModels removeObject:model];
                            break;
                        }
                    }
                
                for (NSInteger idx = 0; idx < self->_selectedVideoModels.count; idx++) {
                    
                    if ([((LLImagePickerModel *)self->_selectedVideoModels[idx]) isEqual:model]) {
                        [self->_selectedVideoModels removeObject:model];
                    }
                }
            }
            [self->_mediaArray removeObjectAtIndex:cellIndexPath.row];
    ...
    if (indexPath.row == _mediaArray.count) {
        ...
        switch (_type) {
            ...
            case LLImageTypeVideo:
                [self openVideo];
                break;
            case LLImageTypeVideoAndTape:
            {
                LLActionSheetView *alert = [[LLActionSheetView alloc]initWithTitleArray:@[@"从视频中选择",@"录像"] andShowCancel: YES];
                alert.ClickIndex = ^(NSInteger index) {
                    if (index == 1){
                        [weakSelf openVideo];
                    } else if (index == 2) {
...
- (void)openVideotape {
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        picker.videoQuality = UIImagePickerControllerQualityTypeIFrame960x540; // 录像质量
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto {
    ...
    for (NSInteger index = 0; index < assets.count; index++) {
        ...
        [[LLImagePickerManager manager] getMediaInfoFromAsset:asset completion:^(NSString *name, id pathData) {
            ...
            if (!self->_allowMultipleSelection) {
                // 用数组是否包含来判断是不成功的。。。
                for (LLImagePickerModel *md in self->_selectedImageModels) {
                    // 新方法
                    if ([md isEqual:model] ) {
                        [temp addObject:md];
            ...
            if (index == assets.count - 1) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (!self->_allowMultipleSelection) {
                        //删除公共存在的，剩下的就是已经不存在的
                        [self->_selectedImageModels removeObjectsInArray:temp];
                        //总媒体数组删先除掉不存在，这样不会影响排列的先后顺序
                        [self->_mediaArray removeObjectsInArray:self->_selectedImageModels];
                        //将这次选择的进行赋值，深复制
                        self->_selectedImageModels = [models mutableCopy];
                        //这次选择的删除公共存在的，剩下的就是新添加的
                        [models removeObjectsInArray:temp2];
                        //总媒体数组中在后面添加新数据
                        [self->_mediaArray addObjectsFromArray:models];
                    }else {
                        [self->_selectedImageModels addObjectsFromArray:models];
                        [self->_mediaArray addObjectsFromArray:models];
                    }
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingVideo:(UIImage *)coverImage sourceAssets:(id)asset {
    ...
    [manager requestAVAssetForVideo:asset
        ...
        model.uploadData = vedioData;
        model.image = coverImage;
        model.isVideo = YES;
        model.asset = phAsset;
        model.mediaURL = urlAsset.URL;
        
        dispatch_async(dispatch_get_main_queue(), ^{
            [self->_selectedVideoModels addObject:model];
            [self->_mediaArray addObject:model];
            [self layoutCollection];
        });
    }];
