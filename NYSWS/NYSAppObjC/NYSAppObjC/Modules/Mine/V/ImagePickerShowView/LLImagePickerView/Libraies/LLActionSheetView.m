...
- (void)setUpUI{
    ...
    CGFloat bgHeight;
    if (self.isShow) {
        bgHeight =  CellHeights * self.titleArr.count + (CellHeights + CellSpace);
    }else{
        bgHeight  = CellHeights * self.titleArr.count;
    }
    ...
    btn.tag = 0;
    btn.frame = CGRectMake(0, bgHeight - CellHeights, bgWidth, CellHeights);
    UIImage * highLightImage = [self imageWithColor:[UIColor groupTableViewBackgroundColor] size:CGSizeMake(bgWidth, CellHeights)];
    ...
    for (int i = 0 ; i < self.titleArr.count; i++) {
        ...
        CGFloat btnY;
        if (self.isShow) {
            btnY = (bgHeight - CellHeights - CellSpace)  - CellHeights*(i+1);
        }else{
            btnY = bgHeight - CellHeights*(i+1);
        }
        CGFloat btnW = bgWidth;
        CGFloat btnH = CellHeights - 0.5f;
