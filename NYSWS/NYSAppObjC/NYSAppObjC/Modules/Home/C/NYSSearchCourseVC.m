...
- (void)setupSearchView {
    ...
    UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, searchViewH + 2 * NNormalSpace)];
    UIView *searchView = [[UIView alloc] initWithFrame:CGRectMake(NNormalSpace, NNormalSpace, kScreenWidth - 2 * NNormalSpace, searchViewH)];
...
- (void)footerRereshing {
    ...
    [NYSNetRequest jsonNetworkRequestWithMethod:@"POST"
                                      ...
                                      argument:argument
                                             remark:@"课程搜索列表"
                                            success:^(id response) {
        NSArray *array = [NSArray modelArrayWithClass:[NYSCourseModel class] json:response[@"records"]];
        if (array.count > 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"NYSCourseCell" owner:self options:nil] firstObject];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NYSCourseModel *model = self.dataSourceArr[indexPath.row];
