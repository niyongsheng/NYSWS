...
- (instancetype)init {
    ...
    if (self) {
        ...
        if(self.animationType!=0){
            self.animationType = self.animationType;
        }
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self){
        _percentFilled = 0;
        self.animationType = NONE;
        if (!self.filledBackgroundColor){
            self.filledBackgroundColor = [UIColor blackColor];
        }
...
- (void)createFillingLoading {
    ...
    layer.backgroundColor = [self.filledBackgroundColor CGColor];
...
- (void)createCircleAndTick {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.1 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.5 animations:^{
            ...
            self.backgroundColor = self.filledBackgroundColor;
            [self layoutIfNeeded];
...
- (void)removeCircleLoadingLayer {
    ...
    NYSLoadingButton __weak *weakSelf = self;
    [UIView animateWithDuration:0.5 animations:^{
...
- (void)createCircleLoadingLayer {
    ...
    self.circleStrokeLoadingLayer.strokeColor = [self.loadingColor CGColor];
